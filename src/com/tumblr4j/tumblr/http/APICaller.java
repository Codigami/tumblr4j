...
public class APICaller implements APICallerInterface {
	...
	private synchronized static HttpClient getHttpClient() {
		if(null==httpClient){
			...
			try {
				InputStream inputStream = ClassLoader.getSystemResourceAsStream("tumblr4j.properties");
	...
	public Map<String, Object> getData(String url, NameValuePair[] nameValuePairs) throws TumblrException{
        ...
        while (retry > 0){
          try {
            ...
            retry --;
            if(retry <= 0){
              throw new TumblrException("IO Exception while calling facebook!", ex);
            }
          }
        }
    ...
    if (statusCode != HttpStatus.SC_OK) {
      ...
      throw new TumblrException(JSONToObjectTransformer.getError(response, statusCode));
    }
